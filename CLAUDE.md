...
- Código do dia (`src/components/CodigoDoDia.jsx`, dados em `src/data/codigosDoDia.js`): escolhido com `diaDoAno() % total` (`src/utils/diaDoAno.js`, igual ao meu-site); desafio "Qual é a saída?" com a certa inserida na posição `indice % 3` entre as 2 `erradas`; Executar usa o mesmo `executarPython`. Conferido por `npm run teoria`.
- Colorir Python: `src/utils/colorirPython.js` devolve linhas de pedaços `{ texto, classes }`; `LinhaColorida` (em `BlocoCodigo.jsx`) desenha; as cores ficam em `.codigo-escuro .tok-*` no `App.css`.
- Páginas `Questao`, `Teoria` e `Licao` e o componente `CodigoDoDia` são carregados com `React.lazy`.
